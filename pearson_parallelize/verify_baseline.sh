...
do
    ...
    if [ $ret -eq 2 ]; then
        echo "${red}ERROR: Significant mismatch found in size ${size}.${reset}"
        errors_found=1
    elif [ $ret -eq 1 ]; then
        echo "${yellow}WARNING: Minor differences found in size ${size}.${reset}"
        warnings_found=1
    elif [ $ret -eq 0 ]; then
        echo "${green}Success: Files match for size ${size}.${reset}"
    else
        echo "${red}ERROR: An unexpected error occurred while processing size ${size}.${reset}"
        errors_found=1
    fi
