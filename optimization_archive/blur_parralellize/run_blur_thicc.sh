...
for threads in "${thread_counts[@]}"; do
    ...
    # run valgrind for all images and thread counts
    for threads in "${thread_counts[@]}"; do
        for img in "${images[@]}"; do
            output="output_${img}_threads_${threads}.ppm"
            echo "Running valgrind (callgrind) on $img with $threads threads"
            MALLOC_MMAPTHRESHOLD=0 valgrind --tool=callgrind ./blur_par 15 "data/$img" "$output" "$threads"
            rm -f "$output" # Remove the Valgrind output file
            echo "-----------------------------------------"
        done
    done
done