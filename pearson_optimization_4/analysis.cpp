...
std::vector<double> correlation_coefficients(std::vector<Vector> datasets)
{
    ...
    for (size_t sample1 = 0; sample1 < n - 1; sample1++) 
    {
        ...
        {
            auto x_mm = datasets[sample1] - means[sample1];
            auto y_mm = datasets[sample2] - means[sample2];

            auto x_mm_over_x_mag = x_mm / x_mm.magnitude();
            auto y_mm_over_y_mag = y_mm / y_mm.magnitude();
